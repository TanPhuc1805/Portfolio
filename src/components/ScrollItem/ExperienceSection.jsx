        ...
        >
          <div className="flex justify-between items-center mb-4">
            ...
            <div className="flex items-center">
              <label className="block text-xl font-[K2D] font-bold mb-2 mr-2 color-gradient">Rating:</label>
              <input
                type="number"
                step="0.1"
                min="0"
                ...
                className="w-16 font-[K2D] p-2 border rounded border-gradient-3"
              />
            </div>
          ...
          <form onSubmit={handleSubmit}>
            <div className="mb-4">
              <label className="block font-[K2D] text-sm font-bold mb-2">Name:</label>
              <div className="flex border-gradient-3">
                ...
                <button
                  type="button"
                  className="bg-blue-500 font-[K2D] text-white px-4 py-2 rounded-r flex items-center"
                >
                  <span className="mr-2 font-[K2D]">LinkedIn</span>
            ...
            <div className="mb-4">
              <label className="block text-sm font-[K2D] font-bold mb-2">Position:</label>
              <input
                type="text"
                value={position}
                onChange={(e) => setPosition(e.target.value)}
                className="w-full p-2 border font-[K2D] rounded border-gradient-3"
                placeholder="Your position"
              />
            </div>
            <div className="mb-4">
              <label className="block text-sm font-[K2D] font-bold mb-2">Evaluation:</label>
              <textarea
                value={evaluation}
            ...
            <div className="flex justify-center w-full mt-4">
              <button
                type="submit"
                className="font-bold font-[K2D] border-2 border-purple-600 text-black px-4 py-2 rounded-lg hover:bg-purple-700 transition duration-300"
              >
                Evaluate
              </button>
            </div>
        ...
        >
          <div className="flex justify-between items-center align-center mb-6">
            <div className="flex items-center">
              ...
              <div>
                <h2 className="text-2xl font-[K2D] font-bold color-gradient">{experience.jobTitle}</h2>
                <p className="font-bold font-[K2D] color-gradient-2">{experience.period}</p>
              </div>
            </div>
            <div className="mb-4">
              <h2 className="text-xl font-bold font-[K2D] color-gradient-2">Rating: 4.7/5</h2>
            </div>
            <div className="mb-4">
              <button 
                className="bg-gradient text-white font-[K2D] font-bold px-4 py-2 rounded-lg mt-2"
            ...
            {[
              ...
              <div key={index} className="border-gradient-2 rounded-lg p-4">
                <div className="flex items-center mb-2">
                  ...
                  <div>
                    <h2 className="font-bold font-[K2D] color-gradient" style={{ fontSize: '16px' }}>{reviewer.name}</h2>
                    <p className="font-bold font-[K2D] color-gradient-2" style={{ fontSize: '13px' }}>{reviewer.role}</p>
                  </div>
                </div>
                <p className="font-bold font-[K2D] color-gradient-2">Rating: {reviewer.rating}</p>
                <p className="mt-2 font-[K2D]">Lorem ipsum, dolor sit amet con sectetur adipi sicing elit. Rerum praes entium eaque eum velit quia? Ipsum sequi, repell endus aspe rnatur inventore nobis facere nisi nes ciunt eveniet, magni quisquam ab quis! Nobis, officiis.</p>
                <p className="mt-2 font-[K2D] color-gradient-2 font-bold text-end">{reviewer.date}</p>
              </div>
            ))}
          </div>
        </motion.div>
    ...
    return (
      ...
      >
        <div className="flex items-center mb-4">
          <img src={experience.logo} alt={experience.company} className="w-16 object-contain mr-4" />
          <div>
            <h6 className="font-bold color-gradient font-[K2D]" style={{ fontSize: '18px' }}>{experience.jobTitle}</h6>
            <p className="font-bold color-gradient-2 font-[K2D]" style={{ fontSize: '13px' }}>{experience.period}</p>
          </div>
        </div>
        <p className="mb-2 text-gray-700 font-bold font-[K2D]" style={{ fontSize: '13px' }}>{experience.description}</p>
        <div className="flex justify-center w-full mt-4">
          <button 
            className="font-bold border-2 font-[K2D] border-purple-600 px-6 py-2 rounded-lg hover:bg-purple-600 hover:text-white transition duration-300"
            style={{ fontSize: '13px' }}
            onClick={(e) => {
              e.stopPropagation();
  ...
  return (
    <div id="exp" className="w-full h-full relative">
      <Section style={{ width: '100%', position: 'relative' }}>
        ...
        <h2 className="text-4xl font-[K2D] font-bold text-white">
          Experiences
        </h2>
                    ...
                    <AnimatePresence>
                      {selectedExperience === exp ? (
                        ...
                        >
                          ...
                          <strong className="text-white font-[K2D] text-center text-lg block mt-2">{exp.period}</strong>
                        </motion.div>
                      )}
                    </AnimatePresence>
                  </div>
